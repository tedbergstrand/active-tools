  ...
  return (
    <div className="space-y-6">
      <Header title="Training">
        <Button variant="traditional" onClick={() => navigate('/log/traditional')}><Plus size={18} /> Log Session</Button>
      </Header>

      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        <StatCard icon={Activity} label="Sessions" value={summary?.totalWorkouts ?? '—'} sublabel="Last 30 days" color="text-emerald-400" />
        ...
        <StatCard icon={TrendingUp} label="Avg RPE" value={summary?.avgRpe ?? '—'} color="text-emerald-400" />
      </div>
      ...
      <div>
        ...
        <div className="space-y-3">
          ...
          {!loading && workouts.length === 0 && (
            <EmptyState icon={Dumbbell} title="No training sessions yet" description="Log your first training session">
              <Button variant="traditional" onClick={() => navigate('/log/traditional')}><Plus size={16} /> Log Session</Button>
            </EmptyState>
          )}
        </div>
      </div>
